...
上面那支示例是：**60.0 秒、1080×1920、10 個鏡頭各 6 秒、20 段字幕**，保留素材原聲並正規化到 −16 LUFS。
這些值來自輸入的 `output` 區塊——**鏡頭數是 `duration ÷ shot_seconds`、字幕段數是 `duration ÷ caption_seconds`，都是除出來的，不另外指定**。
片長是呼叫方給的輸入，agent 不可自行更改。
