...
依此回頭用階段 I 的判準複驗當輪的四項修補：ffprobe 補位與 Cloudflare 重試「不寫會做錯」成立，保留；選鏡窗口驗證只是近失不是失敗，三句加一段指令壓成一句；`check` 的 info 提示「不寫也不會做錯」，**刪除**。同時把階段 I 那條煞車規則搬進 `CLAUDE.md`——留在 RECIPE 等於煞車不在會被讀到的地方。淨效果：該輪 `SKILL.md` 成長從 +7.8% 降到 +5.0%，`VERIFY.md` 回到原狀。
...
4. `SKILL.md` 單輪成長 7.8%（階段 J）。目前只有「加」的機制、沒有「刪」的機制，需要一個定期回收的做法。
